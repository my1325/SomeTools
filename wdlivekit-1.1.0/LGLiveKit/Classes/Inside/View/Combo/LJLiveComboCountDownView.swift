...
class LJLiveComboCountDownView: UIView {
    let imageView:UIImageView = {
        if #available(iOS 13.0, *) {
            let view = UIImageView(image: UIImage(named: "lj_icon_combo", in: Bundle(for: LJLiveComboCountDownView.self), with: nil))
            return view
        } else {
            let view = UIImageView(image: UIImage(named: "lj_icon_combo"))
            return view
        }
    }()
    ...
    func addCountDownAnimation() -> Void {
        ...
        countDownTimer = Timer.scheduledTimer(withTimeInterval: animation.duration, repeats: false, block: { _ in
            if let comboView = self.superview as? LJLiveComboView {
...
extension LJLiveComboCountDownView : CAAnimationDelegate {
    func animationDidStop(_ anim: CAAnimation, finished flag: Bool) {
        if UIApplication.shared.applicationState != .active {
            if let comboView = superview as? LJLiveComboView {
                comboView.finishCountDown()
            }
        }else if flag {
            if let comboView = superview as? LJLiveComboView {
