...
#import "LJLiveIndicatorView.h"

static CGFloat kLCHalfNumber = 0.5f;
static CGFloat kLCMultiple = 1.4f;
@interface LJLiveAnimatedPageControl ()<UIScrollViewDelegate>
...
@implementation LJLiveAnimatedPageControl
...
//        }
//        if (_pageStyle == LJLiveScaleColorPageStyle) {
...
- (void)configCurrentIndicator{
    ...
    if ([view isKindOfClass:[LJLiveIndicatorView class]]) {
        [(LJLiveIndicatorView *)view frontView].layer.timeOffset = 1.0f;
    }
    else{
        view.layer.timeOffset = 0.0f;
    }
...
- (void)setPageStyle:(PageStyle)pageStyle{
    ...
    if (_pageStyle == LJLiveSquirmPageStyle) {
...
- (void)setNumberOfPages:(NSInteger)numberOfPages{
    ...
    if (difference && self.superview) {
        ...
        if (_pageStyle == LJLiveDepthColorPageStyle) {
            [self configDepthView];
        }
        else if (_pageStyle == LJLiveSquirmPageStyle) {
