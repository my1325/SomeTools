...
- (CGRect)imageRectForContentRect:(CGRect)contentRect {
    ...
    switch (self.imagePosition) {
        ...
        case LJLiveItemImagePositionDefault: { // 图片在左
...
@implementation LJLivePageMenu


#pragma mark - public

+ (instancetype)pageMenuWithFrame:(CGRect)frame trackerStyle:(LJLivePageMenuTrackerStyle)trackerStyle {
    LJLivePageMenu *pageMenu = [[LJLivePageMenu alloc] initWithFrame:frame trackerStyle:trackerStyle];
    return pageMenu;
}
...
- (void)buttonInPageMenuClicked:(LJPageMenuButton *)sender {
    ...
    if (self.selectedButton != sender) {
        ...
        if (self.trackerStyle == LJLivePageMenuTrackerStyleTextZoom || _selectedItemZoomScale != 1) {
            if (labs(toIndex-fromIndex) >= 2) { // 该条件意思是当外界滑动scrollView连续的滑动了超过2页
                for (LJPageMenuButton *button in self.buttons) { // 必须遍历将非选中按钮还原缩放，而不是仅仅只让上一个选中的按钮还原缩放。因为当用户快速滑动外界scrollView时，会频繁的调用-zoomForTitleWithProgress:fromButton:toButton:方法，有可能经过的某一个button还没彻底还原缩放就直接过去了，从而可能会导致该按钮文字会显示不全，所以在这里，将所有非选中的按钮还原缩放
...
- (void)setTrackerStyle:(LJLivePageMenuTrackerStyle)trackerStyle {
    ...
    switch (trackerStyle) {
        case LJLivePageMenuTrackerStyleLine:
        case LJLivePageMenuTrackerStyleLineLongerThanItem:
        case LJLivePageMenuTrackerStyleLineAttachment:
            self.tracker.backgroundColor = _selectedItemTitleColor;
            break;
        case LJLivePageMenuTrackerStyleRoundedRect:
        case LJLivePageMenuTrackerStyleRect:
            ...
            _trackerHeight = 0;
            break;
        case LJLivePageMenuTrackerStyleTextZoom:
            // 此样式下默认1.3
...
- (void)layoutSubviews {
    ...
    for (int i= 0 ; i < self.buttons.count; i++) {
        ...
        } else if (button.currentTitle.length && button.currentImage && (button.imagePosition == LJLiveItemImagePositionRight || button.imagePosition == LJLiveItemImagePositionLeft || button.imagePosition == LJLiveItemImagePositionDefault)) {
            contentW = textW + imageW + button.imageTitleSpace+button.contentEdgeInsets.left+button.contentEdgeInsets.right;
        } else if (button.currentTitle && button.currentImage && (button.imagePosition == LJLiveItemImagePositionTop || button.imagePosition == LJLiveItemImagePositionBottom)) {
    ...
    if (self.permutationWay == LJLivePageMenuPermutationWayNotScrollAdaptContent && diff < 0) {
    ...
    [self.buttons enumerateObjectsUsingBlock:^(LJPageMenuButton *button, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (self.permutationWay == LJLivePageMenuPermutationWayScrollAdaptContent) {
        ...
        } else if (self.permutationWay == LJLivePageMenuPermutationWayNotScrollEqualWidths) {
            // 求出外界设置的按钮宽度之和
...
+ (instancetype)itemWithTitle:(NSString *)title image:(UIImage *)image {
    LJLivePageMenuButtonItem *item = [[LJLivePageMenuButtonItem alloc] initWithTitle:title image:image imagePosition:LJLiveItemImagePositionDefault];
    return item;
}

+ (instancetype)itemWithTitle:(NSString *)title image:(UIImage *)image imagePosition:(LJLiveItemImagePosition)imagePosition {
    LJLivePageMenuButtonItem *item = [[LJLivePageMenuButtonItem alloc] initWithTitle:title image:image imagePosition:imagePosition];
    return item;
}
