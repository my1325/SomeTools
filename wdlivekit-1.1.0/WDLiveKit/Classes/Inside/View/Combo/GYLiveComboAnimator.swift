...
@objcMembers open class GYLiveComboAnimator: NSObject {
    public static let shared = GYLiveComboAnimator()
    ...
    public func startComboAnimation(image:UIImage? = nil,imageSize size:CGSize = CGSize(width: 40.0, height: 40.0),containerView view:UIView,count:Int = 5,isSelf:Bool = false) -> Void {
        ...
        if let unpackImage = image {
            ...
            if #available(iOS 13.0, *) {
                giftImage = UIImage(named: "dr_icon_combo_default_image", in: Bundle(for: GYLiveComboCountDownView.self), with: nil) ?? UIImage()
            } else {
