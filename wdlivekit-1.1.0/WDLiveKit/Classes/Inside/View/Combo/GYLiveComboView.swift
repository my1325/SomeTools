...
public protocol GYLiveComboViewDelegate {
    func fb_comboViewDidEndCountDown(_ comboView:GYLiveComboView)
}

@objcMembers public class GYLiveComboView: UIView {
    public var roomId:Int = 0
    public var giftId:Int = 0
    
    public var delegate:GYLiveComboViewDelegate?
    ...
    let rippleView = GYLiveComboRippleView()
    let countDownView = GYLiveComboCountDownView()
    var isQuick = false
    
    let countBarView:UIImageView = {
        if #available(iOS 13.0, *) {
            let view = UIImageView(image: UIImage(named: "fb_bg_count_bar", in: Bundle(for: GYLiveComboCountDownView.self), with: nil))
            return view
        } else {
            let view = UIImageView(image: UIImage(named: "fb_bg_count_bar"))
            return view
        }
    }()
    ...
    let goodView:UIImageView = {
        if #available(iOS 13.0, *) {
            let view = UIImageView(image: UIImage(named: "fb_icon_combo_good", in: Bundle(for: GYLiveComboCountDownView.self), with: nil))
            view.isHidden = true
            return view
        } else {
            let view = UIImageView(image: UIImage(named: "fb_icon_combo_good"))
            view.isHidden = true
            return view
        }
    }()
    ...
    func finishCountDown() -> Void {
        delegate?.fb_comboViewDidEndCountDown(self)
    }
}
