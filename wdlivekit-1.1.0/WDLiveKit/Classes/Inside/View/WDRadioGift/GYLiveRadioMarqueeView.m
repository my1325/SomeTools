...
@interface GYLiveRadioMarqueeView () <GYMarqueeViewTouchResponder>

@property (nonatomic, strong) UIView *contentView;
@property (nonatomic, assign) NSInteger visibleItemCount;
@property (nonatomic, strong) NSMutableArray<GYMarqueeItemView*> *items;
@property (nonatomic, assign) int firstItemIndex;
@property (nonatomic, assign) int dataIndex;
@property (nonatomic, strong) NSTimer *scrollTimer;
@property (nonatomic, strong) GYMarqueeViewTouchReceiver *touchReceiver;
...
@implementation GYLiveRadioMarqueeView
...
- (void)resetAll {
    ...
    if (_direction == GYMarqueeViewDirectionLeftward) {
    ...
    for (int i = 0; i < _visibleItemCount + 2; i++) {
        GYMarqueeItemView *itemView = [[GYMarqueeItemView alloc] init];
...
- (void)createItemView:(GYMarqueeItemView*)itemView {
...
- (void)updateItemView:(GYMarqueeItemView*)itemView atIndex:(NSInteger)index {
    if (index < 0) {
        [itemView clear];
    }
...
- (void)resetTouchReceiver {
    if (_touchEnabled) {
        if (!_touchReceiver) {
            self.touchReceiver = [[GYMarqueeViewTouchReceiver alloc] init];
...
- (void)touchesEndedAtPoint:(CGPoint)point {
    for (GYMarqueeItemView *itemView in _items) {
...
@implementation GYMarqueeViewTouchReceiver
...
@implementation GYMarqueeItemView
