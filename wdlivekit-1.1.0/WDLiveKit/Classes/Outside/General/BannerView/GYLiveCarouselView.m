...
@interface GYLiveCarouselView()<UIScrollViewDelegate>
//轮播的图片数组
...
//分页控件
@property (nonatomic, strong) GYLiveAnimatedPageControl *pageControl;
//当前显示的imageView
...
@implementation GYLiveCarouselView
#pragma mark- 初始化方法
//创建用来缓存图片的文件夹
+ (void)initialize {
    cache = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"GYLiveCarousel"];
    BOOL isDir = NO;
...
//- (GYLiveAnimatedPageControl *)pageControl {
//    if (!_pageControl) {
//
//
//    }
...
- (void)setImageArray:(NSArray *)imageArray{
    ...
    for (int i = 0; i < imageArray.count; i++) {
        if ([imageArray[i] isKindOfClass:[UIImage class]]) {
            ...
            else [_images addObject:[UIImage imageNamed:@"GYLivePlaceholder" inBundle:[NSBundle bundleForClass:[self class]] compatibleWithTraitCollection:nil]];
    ...
    self.pageControl = [[GYLiveAnimatedPageControl alloc] init];
    ...
    self.pageControl.pageStyle = GYLiveScaleColorPageStyle;
