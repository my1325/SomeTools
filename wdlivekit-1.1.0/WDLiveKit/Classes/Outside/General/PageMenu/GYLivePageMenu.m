...
@interface GYPageMenuLine : UIImageView
@property (nonatomic, copy) void(^hideBlock)(void);

@end

@implementation GYPageMenuLine
...
@interface GYPageMenuButton : UIButton

- (instancetype)initWithImagePosition:(GYLiveItemImagePosition)imagePosition;

@property (nonatomic) GYLiveItemImagePosition imagePosition; // 图片位置
@property (nonatomic, assign) CGFloat imageTitleSpace; // 图片和文字之间的间距

@end

@implementation GYPageMenuButton
...
- (void)initialize {
    _imagePosition = GYLiveItemImagePositionLeft;
...
- (CGRect)titleRectForContentRect:(CGRect)contentRect {
    ...
    switch (self.imagePosition) {
        case GYLiveItemImagePositionLeft:
        case GYLiveItemImagePositionDefault: {
...
- (void)setImagePosition:(GYLiveItemImagePosition)imagePosition {
...
@interface GYLivePageMenu()
@property (nonatomic, assign) GYLivePageMenuTrackerStyle trackerStyle;
...
@property (nonatomic, weak) GYPageMenuButton *functionButton;
@property (nonatomic, strong) NSMutableArray *buttons;
@property (nonatomic, strong) GYPageMenuButton *selectedButton;
...
@implementation GYLivePageMenu


#pragma mark - public

+ (instancetype)pageMenuWithFrame:(CGRect)frame trackerStyle:(GYLivePageMenuTrackerStyle)trackerStyle {
    GYLivePageMenu *pageMenu = [[GYLivePageMenu alloc] initWithFrame:frame trackerStyle:trackerStyle];
    return pageMenu;
}

- (instancetype)initWithFrame:(CGRect)frame trackerStyle:(GYLivePageMenuTrackerStyle)trackerStyle {
    if (self = [super init]) {
        self.frame = frame;
...
- (void)setItems:(NSArray *)items selectedItemIndex:(NSInteger)selectedItemIndex {
    ...
    if (self.buttons.count) {
        for (GYPageMenuButton *button in self.buttons) {
    ...
    for (int i = 0; i < items.count; i++) {
        id object = items[i];
        NSAssert([object isKindOfClass:[NSString class]] || [object isKindOfClass:[UIImage class]] || [object isKindOfClass:[GYLivePageMenuButtonItem class]], @"items中的元素类型只能是NSString、UIImage或GYPageMenuButtonItem");
    ...
    if (self.buttons.count) {
        ...
        GYPageMenuButton *selectedButton = [self.buttons objectAtIndex:selectedItemIndex];
        [self buttonInPageMenuClicked:selectedButton];

        // GYLivePageMenuTrackerStyleTextZoom和GYLivePageMenuTrackerStyleNothing样式跟tracker没有关联
...
- (void)insertItem:(GYLivePageMenuButtonItem *)item atIndex:(NSUInteger)itemIndex animated:(BOOL)animated {
    self.insert = YES;
    NSAssert(itemIndex <= self.items.count, @"itemIndex超过了items的总个数“%ld”",self.items.count);
    NSMutableArray *objects = self.items.mutableCopy;
...
- (void)removeItemAtIndex:(NSUInteger)itemIndex animated:(BOOL)animated {
    ...
    if (itemIndex < self.buttons.count) {
        GYPageMenuButton *button = [self.buttons objectAtIndex:itemIndex];
...
- (void)removeAllItems {
    ...
    for (int i = 0; i < self.buttons.count; i++) {
        GYPageMenuButton *button = self.buttons[i];
        [button removeFromSuperview];
    }
...
- (void)setItem:(GYLivePageMenuButtonItem *)item forItemIndex:(NSUInteger)itemIndex {
    if (item == nil) return;
    if (itemIndex < self.buttons.count) {
        GYPageMenuButton *button = [self.buttons objectAtIndex:itemIndex];
...
- (void)setItem:(GYLivePageMenuButtonItem *)item forItemAtIndex:(NSUInteger)itemIndex {
    [self setItem:item forItemIndex:itemIndex];
}

- (GYLivePageMenuButtonItem *)itemAtIndex:(NSUInteger)itemIndex {
    if (itemIndex < self.items.count) {
        id object = [self.items objectAtIndex:itemIndex];
        NSAssert([object isKindOfClass:[GYLivePageMenuButtonItem class]],@"itemIndex对应的item不是GYPageMenuButtonItem类型，请仔细核对");
        return object;
    }
    return nil;
}
...
- (void)setContent:(id)content forItemIndex:(NSUInteger)itemIndex {
    ...
    if (itemIndex < self.buttons.count) {
        ...
        } else if ([content isKindOfClass:[GYLivePageMenuButtonItem class]]) {
            GYLivePageMenuButtonItem *item = (GYLivePageMenuButtonItem *)content;
            [button setTitle:item.title forState:UIControlStateNormal];
            [button setImage:item.image forState:UIControlStateNormal];
            button.imagePosition = item.imagePosition;
            button.imageTitleSpace = item.imageTitleSpace;
...
- (void)setFunctionButtonWithItem:(GYLivePageMenuButtonItem *)item forState:(UIControlState)state {
...
- (void)setTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(GYLiveItemImagePosition)imagePosition imageRatio:(CGFloat)ratio forItemIndex:(NSUInteger)itemIndex {
    if (itemIndex < self.buttons.count) {
        GYPageMenuButton *button = [self.buttons objectAtIndex:itemIndex];
        [button setTitle:title forState:UIControlStateNormal];
        [button setImage:image forState:UIControlStateNormal];
...
- (void)setTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(GYLiveItemImagePosition)imagePosition imageRatio:(CGFloat)ratio imageTitleSpace:(CGFloat)imageTitleSpace forItemIndex:(NSUInteger)itemIndex {
    if (itemIndex < self.buttons.count) {
        GYPageMenuButton *button = [self.buttons objectAtIndex:itemIndex];
        [button setTitle:title forState:UIControlStateNormal];
        [button setImage:image forState:UIControlStateNormal];
...
- (void)setFunctionButtonTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(GYLiveItemImagePosition)imagePosition imageRatio:(CGFloat)ratio forState:(UIControlState)state {
...
- (void)setFunctionButtonTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(GYLiveItemImagePosition)imagePosition imageRatio:(CGFloat)ratio imageTitleSpace:(CGFloat)imageTitleSpace forState:(UIControlState)state {
    [self.functionButton setTitle:title forState:state];
    [self.functionButton setImage:image forState:state];
    self.functionButton.imagePosition = imagePosition;
...
- (void)addButton:(NSInteger)index object:(id)object animated:(BOOL)animated {
    ...
    GYPageMenuButton *button = [GYPageMenuButton buttonWithType:UIButtonTypeCustom];
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        GYLivePageMenuButtonItem *item = (GYLivePageMenuButtonItem *)object;
        [button setTitle:item.title forState:UIControlStateNormal];
        [button setImage:item.image forState:UIControlStateNormal];
        button.imagePosition = item.imagePosition;
        button.imageTitleSpace = item.imageTitleSpace;
...
- (BOOL)haveOrNeedsTracker {
    if (self.trackerStyle != GYLivePageMenuTrackerStyleTextZoom && self.trackerStyle != GYLivePageMenuTrackerStyleNothing) {
        return YES;
    }
    return NO;
}
...
- (void)setupSubViews {
    // 必须先添加分割线
    GYPageMenuLine *dividingLine = [[GYPageMenuLine alloc] init];
    ...
    GYLivePageMenuScrollView *itemScrollView = [[GYLivePageMenuScrollView alloc] init];
    ...
    GYPageMenuButton *functionButton = [GYPageMenuButton buttonWithType:UIButtonTypeCustom];
...
// 按钮点击方法
- (void)buttonInPageMenuClicked:(GYPageMenuButton *)sender {
    ...
    if (self.selectedButton != sender) {
        ...
        if (self.trackerStyle == GYLivePageMenuTrackerStyleTextZoom || _selectedItemZoomScale != 1) {
            if (labs(toIndex-fromIndex) >= 2) { // 该条件意思是当外界滑动scrollView连续的滑动了超过2页
                for (GYPageMenuButton *button in self.buttons) { // 必须遍历将非选中按钮还原缩放，而不是仅仅只让上一个选中的按钮还原缩放。因为当用户快速滑动外界scrollView时，会频繁的调用-zoomForTitleWithProgress:fromButton:toButton:方法，有可能经过的某一个button还没彻底还原缩放就直接过去了，从而可能会导致该按钮文字会显示不全，所以在这里，将所有非选中的按钮还原缩放
...
- (void)moveItemScrollViewWithSelectedButton:(GYPageMenuButton *)selectedButton {
...
// 移动跟踪器
- (void)moveTrackerWithSelectedButton:(GYPageMenuButton *)selectedButton {
...
// 功能按钮的点击方法
- (void)functionButtonClicked:(GYPageMenuButton *)sender {
...
- (void)prepareMoveTrackerFollowScrollView:(UIScrollView *)scrollView {
    ...
    switch (self.trackerFollowingMode) {
        case GYLivePageMenuTrackerFollowingModeAlways:
            // 这个方法才开始移动跟踪器
            ...
            break;
        case GYLivePageMenuTrackerFollowingModeHalf:{
            GYPageMenuButton *fromButton;
            GYPageMenuButton *toButton;
            if (progress > 0.5) {
...
- (void)moveTrackerWithProgress:(CGFloat)progress fromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex currentOffsetX:(CGFloat)currentOffsetX beginOffsetX:(CGFloat)beginOffsetX {
    ...
    if (self.trackerStyle == GYLivePageMenuTrackerStyleLine) {
    ...
    } else if (self.trackerStyle == GYLivePageMenuTrackerStyleLineAttachment) {
    ...
    } else if (self.trackerStyle == GYLivePageMenuTrackerStyleTextZoom || self.trackerStyle == GYLivePageMenuTrackerStyleNothing) {
        // 缩放文字
        if (_selectedItemZoomScale != 1) {
            [self zoomForTitleWithProgress:progress fromButton:fromButton toButton:toButton];
        }
    } else if (self.trackerStyle == GYLivePageMenuTrackerStyleRoundedRect) {
        newCenter.x = fromButton.center.x + xDistance * progress;
...
- (void)setTrackerStyle:(GYLivePageMenuTrackerStyle)trackerStyle {
    _trackerStyle = trackerStyle;
    switch (trackerStyle) {
        case GYLivePageMenuTrackerStyleLine:
        case GYLivePageMenuTrackerStyleLineLongerThanItem:
        case GYLivePageMenuTrackerStyleLineAttachment:
            self.tracker.backgroundColor = _selectedItemTitleColor;
            break;
        case GYLivePageMenuTrackerStyleRoundedRect:
        case GYLivePageMenuTrackerStyleRect:
            ...
            _trackerHeight = 0;
            break;
        case GYLivePageMenuTrackerStyleTextZoom:
            // 此样式下默认1.3
...
- (void)setSelectedItemIndex:(NSInteger)selectedItemIndex {
    _selectedItemIndex = selectedItemIndex;
    if (self.buttons.count) {
        GYPageMenuButton *button = [self.buttons objectAtIndex:selectedItemIndex];
        [self buttonInPageMenuClicked:button];
    }
}

- (void)setDelegate:(id<GYLivePageMenuDelegate>)delegate {
    if (delegate == _delegate) {return;}
    _delegate = delegate;
    if (self.buttons.count) {
        GYPageMenuButton *button = [self.buttons objectAtIndex:_selectedItemIndex];
...
- (void)setPermutationWay:(GYLivePageMenuPermutationWay)permutationWay {
    _permutationWay = permutationWay;
    if (!_forceUseSettingSpacing) {
        if (_permutationWay == GYLivePageMenuPermutationWayNotScrollEqualWidths) {
...
- (void)setCloseTrackerFollowingMode:(BOOL)closeTrackerFollowingMode {
    ...
    if (closeTrackerFollowingMode) {
        self.trackerFollowingMode = GYLivePageMenuTrackerFollowingModeEnd;
    } else {
        self.trackerFollowingMode = GYLivePageMenuTrackerFollowingModeAlways;
    }
}
#pragma mark - getter
...
- (void)layoutSubviews {
    ...
    for (int i= 0 ; i < self.buttons.count; i++) {
        ...
        } else if (button.currentTitle.length && button.currentImage && (button.imagePosition == GYLiveItemImagePositionRight || button.imagePosition == GYLiveItemImagePositionLeft || button.imagePosition == GYLiveItemImagePositionDefault)) {
            contentW = textW + imageW + button.imageTitleSpace+button.contentEdgeInsets.left+button.contentEdgeInsets.right;
        } else if (button.currentTitle && button.currentImage && (button.imagePosition == GYLiveItemImagePositionTop || button.imagePosition == GYLiveItemImagePositionBottom)) {
    ...
    if (self.permutationWay == GYLivePageMenuPermutationWayNotScrollAdaptContent && diff < 0) {
    ...
    [self.buttons enumerateObjectsUsingBlock:^(GYPageMenuButton *button, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (self.permutationWay == GYLivePageMenuPermutationWayScrollAdaptContent) {
        ...
        } else if (self.permutationWay == GYLivePageMenuPermutationWayNotScrollEqualWidths) {
            // 求出外界设置的按钮宽度之和
        ...
        // HUANGCHENG
        if (self.flipButtonsByRTL) {
            CGRect rect = button.frame;
            rect.origin.x = button.superview.bounds.size.width - rect.origin.x - rect.size.width;
            button.frame = rect;
        }
    }];
...
- (void)resetupTrackerFrameWithSelectedButton:(GYPageMenuButton *)selectedButton {
    CGFloat trackerX;
    CGFloat trackerY;
    CGFloat trackerW;
    CGFloat trackerH;
...
@implementation GYLivePageMenuButtonItem

+ (instancetype)itemWithTitle:(NSString *)title image:(UIImage *)image {
    GYLivePageMenuButtonItem *item = [[GYLivePageMenuButtonItem alloc] initWithTitle:title image:image imagePosition:GYLiveItemImagePositionDefault];
    return item;
}

+ (instancetype)itemWithTitle:(NSString *)title image:(UIImage *)image imagePosition:(GYLiveItemImagePosition)imagePosition {
    GYLivePageMenuButtonItem *item = [[GYLivePageMenuButtonItem alloc] initWithTitle:title image:image imagePosition:imagePosition];
    return item;
}

- (instancetype)initWithTitle:(NSString *)title image:(UIImage *)image imagePosition:(GYLiveItemImagePosition)imagePosition {
    if (self = [super init]) {
        self.title = title;
        self.image = image;
        self.imagePosition = imagePosition;
