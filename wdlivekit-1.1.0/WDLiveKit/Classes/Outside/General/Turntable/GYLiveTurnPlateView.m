...
#import "GYLiveTurntableView.h"
#import "GYLiveAutoScrollLabel.h"

@interface GYLiveTurnPlateView() <CAAnimationDelegate>

@property (weak, nonatomic) IBOutlet GYLiveAutoScrollLabel *titleLabel;
...
@property(nonatomic,strong) GYLiveTurntableView *circleView;
...
@property (strong,nonatomic) GYLiveTurntableViewModel * curItem;
...
@implementation GYLiveTurnPlateView
...
-(GYLiveTurntableView*)circleView {
    
    if (_circleView == nil) {
        _circleView = [[GYLiveTurntableView alloc] init];
        ...
        kGYWeakSelf;
        [_circleView setLunckyAnimationDidStopBlock:^(BOOL flag, GYLiveTurntableViewModel *item) {
...
- (void)initItemDataArray
{
    ...
    self.titleLabel.font = kGYHurmeBoldFont(14);//字体大小
    ...
    self.titleLabel.scrollDirection = GYLiveAutoScrollDirectionLeft;
    
    self.titleLabel.text = kGYLiveHelper.data.current.turntableTitle;
    ...
    NSArray * items = kGYLiveHelper.data.current.turntableItems;
    for (int i = 0; i < items.count ; i++) {
        GYLiveTurntableViewModel *model = [[GYLiveTurntableViewModel alloc] init];
        if (items.count > i) {
    ...
    [_luckyItemArray sortUsingComparator:^NSComparisonResult(GYLiveTurntableViewModel *  _Nonnull obj1, GYLiveTurntableViewModel *  _Nonnull obj2) {
...
- (GYLiveTurntableViewModel *)getItemByIndex:(NSInteger)index
{
    ...
    GYLiveTurntableViewModel * item = result.firstObject;
    return item;
}
...
-(void)startAction {
    ...
    if (self.curItem) {
        GYLiveTurntableViewModel * item = [self getItemByIndex:self.curItem.index];
        if (item) {
            GYLog(@"奖品应该是：%@",item);
...
-(void)reciveTurnPlateInfoData:(NSDictionary*)dic {
    ...
    //转盘开关
    kGYLiveHelper.data.current.turntableFlag = [[dic objectForKey:@"turnTableIsOpen"] integerValue];
    //转盘title
    kGYLiveHelper.data.current.turntableTitle = ([[dic objectForKey:@"turnTableTitle"] isKindOfClass:NSNull.class]||[dic objectForKey:@"turnTableTitle"] == nil) ? @"":[dic objectForKey:@"turnTableTitle"];
    //转盘数据
    kGYLiveHelper.data.current.turntableItems = ([[dic objectForKey:@"turnTableInfoList"] isKindOfClass:NSNull.class] || [dic objectForKey:@"turnTableInfoList"] == nil) ? @[]:[dic objectForKey:@"turnTableInfoList"];
    //选中数据
    NSInteger selectedIndex = [[dic objectForKey:@"selectedIndex"] integerValue];
    
    //主播关闭转盘
    if (kGYLiveHelper.data.current.turntableFlag == 2) {
        //隐藏转盘
        self.hiddenBlock();
    }
...
-(NSArray*)getColorsWithItems:(NSArray*)items {
    //偶数
    if (items.count%2 == 0) {
        return @[kGYColorFromRGBA(0xffffff,1.0),kGYColorFromRGBA(0xffffff, 0.9)];
    }else{
        if (items.count == 3) {
            return @[kGYColorFromRGBA(0xffffff, 1.0),kGYColorFromRGBA(0xffffff, 0.9),kGYColorFromRGBA(0xffffff, 0.8)];
        }else if (items.count == 5) {
            return @[kGYColorFromRGBA(0xffffff, 1.0),kGYColorFromRGBA(0xffffff, 0.9),kGYColorFromRGBA(0xffffff, 0.8),kGYColorFromRGBA(0xffffff, 1.0),kGYColorFromRGBA(0xffffff, 0.9)];
        }else if (items.count == 7) {
            return @[kGYColorFromRGBA(0xffffff, 1.0),kGYColorFromRGBA(0xffffff, 0.9),kGYColorFromRGBA(0xffffff, 0.8),kGYColorFromRGBA(0xffffff, 1.0),kGYColorFromRGBA(0xffffff, 0.9),kGYColorFromRGBA(0xffffff, 1.0),kGYColorFromRGBA(0xffffff, 0.8)];
        }else if (items.count == 9){
            return @[kGYColorFromRGBA(0xffffff, 1.0),kGYColorFromRGBA(0xffffff, 0.9),kGYColorFromRGBA(0xffffff, 0.8),kGYColorFromRGBA(0xffffff, 1.0),kGYColorFromRGBA(0xffffff, 0.9),kGYColorFromRGBA(0xffffff, 0.8),kGYColorFromRGBA(0xffffff, 1.0),kGYColorFromRGBA(0xffffff, 0.9),kGYColorFromRGBA(0xffffff, 0.8)];
        }else if(items.count == 11) {
            return @[kGYColorFromRGBA(0xffffff,1.0),kGYColorFromRGBA(0xffffff, 0.9),kGYColorFromRGBA(0xffffff, 0.8),kGYColorFromRGBA(0xffffff, 1.0),kGYColorFromRGBA(0xffffff, 0.9),kGYColorFromRGBA(0xffffff, 0.8),kGYColorFromRGBA(0xffffff, 1.0),kGYColorFromRGBA(0xffffff, 0.9),kGYColorFromRGBA(0xffffff, 0.8),kGYColorFromRGBA(0xffffff, 1.0),kGYColorFromRGBA(0xffffff, 0.9)];
