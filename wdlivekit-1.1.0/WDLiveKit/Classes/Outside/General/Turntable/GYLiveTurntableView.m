...
@implementation GYLiveTurntableGradientColorModel
@end

@implementation GYLiveTurntableViewModel
...
@interface GYLiveTurntableView ()<CAAnimationDelegate>{
...
@implementation GYLiveTurntableView
...
- (void)setLuckyItemArray:(NSArray<GYLiveTurntableViewModel *> *)luckyItemArray{
...
//    angle4Rotate = angle4Rotate+angel;//背景图片自带文字和奖品模式
     ...
     */
    CGFloat radians = GYDegress2Radians(angle4Rotate) + M_PI * 20;
...
{
//    [[NSNotificationCenter defaultCenter] postNotificationName:@"GYLunckyAnimationDidStopNotication" object:nil];
    ...
    if (_luckyItemArray && _luckyItemArray.count) {
        ...
        */
        for (int i = 0; i < count; i++) {
            ...
            GYLiveTurntableViewModel *obj = [_luckyItemArray objectAtIndex:i];
            ...
            [fanPath addArcWithCenter:center
                            radius:center.x - _circleWidth
                        startAngle:GYDegress2Radians(i * degree)
                          endAngle:GYDegress2Radians((i + 1) * degree)
                         clockwise:YES];
            [fanPath closePath];
            if (_panBgGradientColors) {
                [fanPath fill];
                int colorIndex = i%_panBgGradientColors.count;
                GYLiveTurntableGradientColorModel * gradientColorModel = _panBgGradientColors[colorIndex];
            ...
            // 自定义显示...
            NSInteger max = floor(kGYWidthScale(15));
            ...
            [self drawStringOnLayer:self.layer withAttributedText:attributedStr.copy atAngle:GYDegress2Radians((i + 0.5) * degree) withRadius:center.x - _circleWidth - _textFontSize - _textPadding];
            ...
            // DIMOOY
            CGPoint imageLayerPos = pointAroundCircumference(center, (center.x - _circleWidth) / 2.0+10, GYDegress2Radians((i + 0.5) * degree));
            imageLayer.frame = CGRectMake(0, 0, _imageSize.width, _imageSize.height);
            imageLayer.position = imageLayerPos;
            imageLayer.affineTransform = CGAffineTransformRotate(CGAffineTransformIdentity, GYDegress2Radians((i + 0.5) * degree) + M_PI_2);
...
//    if (angle > GYDegress2Radians(10) && angle < GYDegress2Radians(170)) {// 反向 使文字 可读
//        //bottom string
...
//    textSize.height += 1;//高度刚好显示不出第二行，要加1
    ...
    CGFloat flagValue = kGYWidthScale(38.5);
