...
#import "NSString+GYLiveAdd.h"
#import <CommonCrypto/CommonDigest.h>

@implementation NSString (GYLiveAdd)
...
- (NSString *)fb_localized
{
    ...
    NSString *abbr = kGYLiveManager.inside.localizableAbbr;
    if ([supports containsObject:abbr]) {
        NSString *path = [kGYLiveBundle pathForResource:abbr ofType:@"lproj"];
        if (path) {
            NSString *text = [[NSBundle bundleWithPath:path] localizedStringForKey:self value:self table:@"GYLiveLocalizable"];
            return text;
        }
    }
    return self;
